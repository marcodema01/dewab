...
async function main() {
    ...
    function setupDeviceListener(dewabInstance) {
        // Start listening for updates from the specific Arduino device
        dewabInstance.device(ARDUINO_DEVICE_NAME).on('update', (state) => {
            // The event is already being handled by the global event bus listener.
            // This call is primarily to initiate the subscription.
            console.log(`[Demo] Received raw state update from device proxy for ${ARDUINO_DEVICE_NAME}:`, state);
        });
    }
    ...
    connectBtn.addEventListener('click', async () => {
        if (dewab.isConnected()) {
            ...
            try {
                await dewab.connect();
                // After connecting, set up the device listener
                setupDeviceListener(dewab);
            } catch (error) {
    ...
    dewab.eventBus.on(EVENT_TYPES.DEVICE_STATE_UPDATED, async (data) => {
        console.log('[Demo] Received DEVICE_STATE_UPDATED:', data);
        // Check for the correct nested property from the Arduino state
        if (data.device === ARDUINO_DEVICE_NAME && data.state && data.state.inputs && data.state.inputs.button_d2 === true) {
            addMessage('Arduino button pressed! Requesting a Kanye quote...', 'system');
